...
def add_account(user_id: int, phone: str, label: str):
    if DATABASE_URL:
        ...
        execute("INSERT OR REPLACE INTO accounts (user_id, phone, label, active) VALUES (?,?,?,1)",
                (user_id, phone, label))
...
def log_broadcast(user_id, phone, total, ok, fail):
    execute("INSERT INTO broadcasts (user_id,phone,total,ok,fail) VALUES(?,?,?,?,?)",
